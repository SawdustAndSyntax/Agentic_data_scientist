...
from .metrics import evaluate, scorer_name, higher_is_better


def model_stability(model, X: pd.DataFrame, y, *, task: str, metric: str,
                    repeats: int = 8, test_size: float = 0.2,
                    random_state: int = 100, n_jobs: int = 1) -> dict:
    ...
    for i in range(repeats):
        ...
        prob=None
        if metric=='roc_auc':
            if hasattr(m,'predict_proba'): prob=m.predict_proba(Xte)
            elif hasattr(m,'decision_function'): prob=m.decision_function(Xte)
    ...
    score_summary={
        ...
        'coefficient_of_variation':float(abs(scores.score.std(ddof=1)/scores.score.mean())) if len(scores)>1 and scores.score.mean()!=0 else 0.0,
    }
    if imp.empty:
        feature_summary=pd.DataFrame(columns=['feature','importance_mean','importance_std','positive_share','stability_score'])
    else:
        feature_summary=(imp.groupby('feature').importance
            .agg(importance_mean='mean',importance_std='std',positive_share=lambda s: float((s>0).mean()))
            .reset_index())
