...
def qi(x): return x if SAFE_IDENT.match(x) else '`'+x.replace('`','``')+'`'
...
class DatabricksCatalogAdapter(CatalogAdapter):
    ...
    def __init__(self, connection=None, *, executor=None, catalog: str | None=None, schemas: list[str] | None=None):
        self.exec=executor or DBAPIExecutor(connection); self.catalog=catalog; self.schemas=schemas
    def _prefix(self): return f"{qi(self.catalog)}.information_schema" if self.catalog else 'system.information_schema'
    ...
    def entities(self):
        ...
        cols=self.exec.query(f"SELECT table_catalog,table_schema,table_name,column_name,data_type FROM {p}.columns"+self._schema_filter())
        ...
        for _,r in cols.iterrows():
            q=f"{r.table_catalog}.{r.table_schema}.{r.table_name}"
            if q in entities: entities[q].fields.append(SemanticField(str(r.column_name),str(r.data_type)))
    ...
    def relationships(self):
        ...
        try:
            ...
            needed={'source_catalog','source_schema','source_table','source_column','target_catalog','target_schema','target_table','target_column'}
            if needed.issubset(set(rel.columns)):
                for _,r in rel.iterrows():
                    s=f"{r.source_catalog}.{r.source_schema}.{r.source_table}"; t=f"{r.target_catalog}.{r.target_schema}.{r.target_table}"
                    out.append(SemanticRelationship(str(r.get('constraint_name','fk')),s,t,(str(r.source_column),),(str(r.target_column),),'foreign_key',1.0))
        except Exception: pass
        return out
    def sample(self, qualified_name, limit=1000):
        parts=qualified_name.split('.')
        if len(parts)!=3: raise ValueError('Expected catalog.schema.table')
