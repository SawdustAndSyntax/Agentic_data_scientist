...
def semantic_relevance(entity: SemanticEntity, request: DiscoveryRequest) -> tuple[float,list[str]]:
    ...
    reasons=[]
    if h_overlap: reasons.append('hypothesis terms: '+', '.join(sorted(h_overlap)))
    if overlap-h_overlap: reasons.append('context terms: '+', '.join(sorted(overlap-h_overlap)[:8]))
    if entity.description: reasons.append('catalog description available')
