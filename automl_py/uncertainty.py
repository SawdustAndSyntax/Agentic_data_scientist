...
class SplitConformalRegressor:
    ...
    def __init__(self, estimator, alpha: float = 0.10, calibration_size: float = 0.2,
                 random_state: int = 100):
        if not 0 < alpha < 1: raise ValueError('alpha must be between 0 and 1')
        self.estimator=estimator; self.alpha=alpha; self.calibration_size=calibration_size; self.random_state=random_state
    ...
    def fit(self, X: pd.DataFrame, y):
        ...
        try: self.radius_=float(np.quantile(resid,q,method='higher'))
        except TypeError: self.radius_=float(np.quantile(resid,q,interpolation='higher'))
        return self
    ...
    def empirical_coverage(self, X: pd.DataFrame, y) -> float:
        p=self.predict(X); yy=np.asarray(y)
