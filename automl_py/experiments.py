...
def feature_family_value(model_or_factory, X: pd.DataFrame, y, *, cv, metric: str,
                         feature_families: dict[str, list[str]], n_jobs: int = -1) -> pd.DataFrame:
    ...
    for name, cols in feature_families.items():
        ...
        rows.append({'feature_family':name,'columns':','.join(valid),'baseline_scorer':base,'with_family_scorer':score,'scorer_gain':gain,'useful':bool(gain>0)})
    return pd.DataFrame(rows).sort_values('scorer_gain',ascending=False).reset_index(drop=True) if rows else pd.DataFrame(columns=['feature_family','columns','baseline_scorer','with_family_scorer','scorer_gain','useful'])


def compare_candidate_datasets(scientist_factory, base_df: pd.DataFrame, target: str,
                               candidate_feature_sets: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    rows.append({'experiment':'baseline','cv_performance':baseline.cv_performance,'test_performance':baseline.test_performance,'model':baseline.model})
