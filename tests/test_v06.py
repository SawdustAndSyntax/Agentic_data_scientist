...
from automl_py import (
    AutoMLConfig, AutonomousAutoMLScientist,
    FeatureAvailabilityRegistry, SplitConformalRegressor,
...
def test_conformal_regressor():
    ...
    c=SplitConformalRegressor(Pipeline([('imp',SimpleImputer()),('m',Ridge())]),alpha=.1).fit(X,y)
...
def test_autonomous_smoke(tmp_path):
    ...
    cfg=AutoMLConfig(
        task='regression', metric='r2', preprocessors=('original',),
        imputation_strategies=('median',), feature_selection=('none',),
        models=('ridge',), tune=False, cv_folds=3, n_jobs=1,
        stability_repeats=2, save_models=False,
