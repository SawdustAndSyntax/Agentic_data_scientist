...
from automl_py.discovery_agent import (
    SemanticEntity, SemanticField, SemanticRelationship, DiscoveryRequest,
    InMemoryCatalogAdapter, DataDiscoveryAgent, JoinValidator, PredictiveDiscoveryLoop,
    SnowflakeCatalogAdapter,
)


def build_catalog():
    sales=SemanticEntity('sales','analytics.public.sales','memory','table','weekly store ice cream sales',[
        SemanticField('store_id','int','key'),SemanticField('week','date','dimension'),SemanticField('sales','float','metric')
    ])
    store=SemanticEntity('store','analytics.public.store','memory','table','store master geography',[
        SemanticField('store_id','int','key'),SemanticField('weather_station_id','int','key')
    ])
    weather=SemanticEntity('weather_daily','external.weather.daily','memory','table','daily weather temperature humidity precipitation',[
        SemanticField('weather_station_id','int','key'),SemanticField('temperature','float','dimension'),SemanticField('humidity','float','dimension')
    ])
    ...
    rels=[
        ...
        SemanticRelationship('store_weather','analytics.public.store','external.weather.daily',('weather_station_id',),('weather_station_id',)),
    ]
    frames={
        'external.weather.daily':pd.DataFrame({'weather_station_id':[10,20,30],'temperature':[70,80,75],'humidity':[.4,.6,.5]}),
...
def test_predictive_discovery_loop_keeps_uplift():
    adapter=build_catalog(); agent=DataDiscoveryAgent(adapter)
    ...
    r=PredictiveDiscoveryLoop(agent).run(req,anchor_entities=['analytics.public.sales'],base_df=base,candidate_loader=loader,experiment_runner=runner,top_n=1)
...
class FakeExecutor:
    def query(self, sql):
        ...
        if 'semantic_tables' in q:
            return pd.DataFrame([{
                'semantic_view_catalog':'ANALYTICS','semantic_view_schema':'PUBLIC','semantic_view_name':'RETAIL',
                'name':'STORE','base_table_catalog':'ANALYTICS','base_table_schema':'PUBLIC','base_table_name':'STORE',
                'primary_keys':['STORE_ID'],'comment':'store entity'
            },{
                'semantic_view_catalog':'ANALYTICS','semantic_view_schema':'PUBLIC','semantic_view_name':'RETAIL',
                'name':'WEATHER','base_table_catalog':'ANALYTICS','base_table_schema':'PUBLIC','base_table_name':'WEATHER_DAILY',
                'primary_keys':['STATION_ID'],'comment':'weather observations'
            }])
        if 'semantic_dimensions' in q:
            return pd.DataFrame([{
                'semantic_view_catalog':'ANALYTICS','semantic_view_schema':'PUBLIC','semantic_view_name':'RETAIL',
                'table_name':'WEATHER','name':'TEMPERATURE','data_type':'NUMBER','expression':'temperature','synonyms':['TEMP'],'comment':'air temperature'
            }])
        if 'semantic_metrics' in q:
            return pd.DataFrame(columns=['semantic_view_catalog','semantic_view_schema','semantic_view_name','table_name','name','data_type','expression','synonyms','comment'])
        if 'semantic_relationships' in q:
            return pd.DataFrame([{
                'semantic_view_catalog':'ANALYTICS','semantic_view_schema':'PUBLIC','semantic_view_name':'RETAIL','name':'STORE_WEATHER',
                'table_name':'STORE','foreign_keys':['WEATHER_STATION_ID'],'ref_table_name':'WEATHER','ref_keys':['STATION_ID']
            }])
...
def test_snowflake_semantic_adapter():
    ...
    entities=a.entities(); rels=a.relationships()
